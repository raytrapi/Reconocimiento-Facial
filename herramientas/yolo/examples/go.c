...
move pick_move(mcts_tree *tree, float temp, int player)
{
    ...
    */
    ...
    int indexes[10]; //Sustituyo nind por su valor constante ya que se produce un error en Windows
    ...
    while(1){
        ...
        if (!strcmp(buff, "protocol_version")){
            ...
            if(type[0] == 'd' || type[0] == 'D'){
                ...
                FILE *p;
#ifdef _WIN32
                p= _popen("./gnugo --mode gtp < game.txt", "r");
#else
                p= popen("./gnugo --mode gtp < game.txt", "r");
#endif
                for(i = 0; i < count; ++i){
                    free(fgetl(p));
                    free(fgetl(p));
